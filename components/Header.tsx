...
const Header: React.FC<HeaderProps> = ({ session, profile, handleSignOut, onSignInClick, onAccountClick, isAdminView, setIsAdminView }) => {
  const { t } = useSettings();
    ...
    <header className="sticky top-0 z-40 bg-white/70 dark:bg-gray-900/70 backdrop-blur-lg shadow-sm">
      ...
      <div className="container mx-auto px-4 py-2 md:py-0 h-auto md:h-20 flex flex-wrap md:flex-nowrap justify-between items-center">
        ...
        <div className="md:flex-1 md:justify-start">
            {/* Social icons removed for cleaner UI */}
        </div>
        ...
        <div className="flex items-center space-x-2 md:space-x-3 md:flex-1 md:justify-end">
          ...
          {session ? (
            <>
              {profile?.role === 'admin' && <AdminViewToggle isAdminView={isAdminView} setIsAdminView={setIsAdminView} />}
