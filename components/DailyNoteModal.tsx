...
const DailyNoteModal: React.FC<DailyNoteModalProps> = ({ isOpen, onClose, onSave, onDelete, note, date, isSaving, error, readOnly = false }) => {
    ...
    const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (e.target.files && e.target.files[0]) {
            ...
            setRemoveExistingFile(false); // If a new file is chosen, we don't need to remove the old one explicitly yet
        }
    };
    ...
    const handleRemoveFile = () => {
        ...
        setFile(null);
        setFilePreview(null);
        setRemoveExistingFile(true); // Mark existing file for removal
    ...
    const renderError = () => {
        ...
        if (error.includes('Bucket not found')) {
            errorMessage = 'Storage Error: The "daily_attachments" bucket was not found. Please follow the setup instructions in the README.md file to create the necessary database tables and storage buckets.';
        }

        return (
            <div className="bg-red-100 dark:bg-red-900/30 border-l-4 border-red-500 text-red-700 dark:text-red-300 p-3 rounded-md text-sm animate-fadeIn">
                {errorMessage}
            </div>
        );
    }
                ...
                <div className="mt-6 space-y-4">
                    <div>
                        ...
                        <textarea 
                            ...
                            readOnly={readOnly}
                            className="mt-1 block w-full px-3 py-2 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-[var(--accent-color)] focus:border-[var(--accent-color)] sm:text-sm disabled:opacity-70 disabled:bg-gray-100 dark:disabled:bg-gray-700/50"
                            placeholder={readOnly && !noteText ? "No note was added for this day." : "Add details about your work day..."}
                        />
                    </div>
                    <div>
                             ...
                             <div className="mt-2 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 dark:border-gray-600 border-dashed rounded-md">
                                <div className="space-y-1 text-center">
                                    ...
                                    <div className="flex text-sm text-gray-600 dark:text-gray-400">
                                         {!readOnly ? (
                                             <label htmlFor="file-upload" className="relative cursor-pointer bg-white dark:bg-gray-800 rounded-md font-medium text-[var(--accent-color)] hover:text-[var(--accent-color-dark)] focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-[var(--accent-color)]">
                                        ...
                                        {!readOnly && <p className="pl-1">or drag and drop</p>}
                                    </div>
                                    {!readOnly && <p className="text-xs text-gray-500">PNG, JPG, PDF up to 10MB</p>}
                                </div>
                            </div>
                        )}
                    </div>
            ...
            <div className="bg-gray-50 dark:bg-gray-800/50 px-6 py-3 flex flex-col gap-3 rounded-b-2xl">
                ...
                <div className="flex justify-between items-center">
                    {note && !readOnly ? (
                        ...
                        <div></div> // Placeholder to keep right side aligned
                    )}
